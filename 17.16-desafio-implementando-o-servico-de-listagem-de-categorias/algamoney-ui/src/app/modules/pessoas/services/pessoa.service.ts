...
import { PessoaFiltro, Pessoa } from '../models';
...
export class PessoaService extends Service implements IPessoaService {
  ...
  listarTodas = () => this.httpGet(this.urlRersource, this.headers).pipe(map(resp => resp['content']));

  excluir = (codigo: number) => this.httpDelete(this.urlRersource, codigo,  this.headers).pipe(map(() => {}));
  ...
  mudarStatus = (codigo: number, novoStatus: boolean) => {
    ...
    return  this.httpPut(`${this.urlRersource}/${codigo}/ativo`, novoStatus, headers).pipe(map(() => {}));
  }
}
