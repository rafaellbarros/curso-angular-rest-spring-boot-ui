...
export class PessoaService implements IPessoaService {
  ...
  pesquisar = (filtro: PessoaFiltro): Observable<any> => {
    ...
    return this.service.httpGet(`${this.urlResource}`, this.service.headersAuthorization, params).pipe(map(resp => {
  ...
  excluir = (codigo: number): Observable<void> =>
    this.service.httpDelete(this.urlResource, codigo,  this.service.headersAuthorization).pipe(map(() => null))

  mudarStatus = (codigo: number, novoStatus: boolean) => {
    return this.service.httpPut(`${this.urlResource}/${codigo}/ativo`, novoStatus,
    this.service.headersAuthorizationContentTypeJson).pipe(map(() => null));
  }

  adicionar = (pessoa: Pessoa): Observable<Pessoa> => {
    return this.service.httpPost(this.urlResource, pessoa, this.service.headersAuthorizationContentTypeJson);
  }

  atualizar(pessoa: Pessoa): Observable<Pessoa> {
    return this.service.httpPut(`${this.urlResource}/${pessoa.codigo}`, pessoa, this.service.headersAuthorizationContentTypeJson);
  }

  buscarPorCodigo(codigo: number): Observable<Pessoa> {
    return this.service.httpGet(`${this.urlResource}/${codigo}`, this.service.headersAuthorizationContentTypeJson);
  }
}
