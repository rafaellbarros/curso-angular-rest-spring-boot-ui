...
export abstract class Service implements IService {
  ...
  httpGet (urlResource: string, headers?: HttpHeaders) {
    return this._http.get(`${this._baseUrl}/${urlResource}`, { headers: headers != null ? headers :  new HttpHeaders() })
  ...
  httpDelete = (urlResource: string, object: any) => this._http.delete(`${this._baseUrl}/${urlResource}/${object['id']}`)
    .pipe(map(resp => resp), catchError(err =>  throwError(`Error: httpDelete, Status: ${err.status}`)))
